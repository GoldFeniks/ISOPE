...
{
    forward_data_ = static_cast<complex_t*>(fftw_malloc(sizeof(complex_t) * size));
    forward_data_end_ = forward_data_ + size;
    backward_data_ = same_data ? forward_data_ : static_cast<complex_t*>(fftw_malloc(sizeof(complex_t) * size));
...
const isope::fft::basic_fft & isope::fft::fftw::normalize_forward() const {
    std::transform(backward_data_, backward_data_end_, backward_data_,
                   [this](const complex_t& value) { return value / std::sqrt(dsize_); });
    return *this;
}

const isope::fft::basic_fft & isope::fft::fftw::normalize_backward() const {
    std::transform(backward_data_, backward_data_end_, backward_data_,
                   [this](const complex_t& value) { return value / dsize_; });
    return *this;
}

const isope::fft::basic_fft::complex_t* isope::fft::fftw::forward_data() const {
    return forward_data_;
}

const isope::fft::basic_fft::complex_t* isope::fft::fftw::backward_data() const {
    return backward_data_;
}

const isope::fft::basic_fft::complex_t* isope::fft::fftw::forward_data_end() const {
    return forward_data_end_;
}

const isope::fft::basic_fft::complex_t* isope::fft::fftw::backward_data_end() const {
    return backward_data_end_;
}

int isope::fft::fftw::size() const {
    return size_;
}
...
isope::fft::basic_fft::complex_t* isope::fft::fftw::forward_data() {
    return forward_data_;
}

isope::fft::basic_fft::complex_t* isope::fft::fftw::backward_data() {
    return backward_data_;
}

isope::fft::basic_fft::complex_t& isope::fft::fftw::forward_data(const size_t &index) {
    return forward_data_[index];
}

const isope::fft::basic_fft::complex_t& isope::fft::fftw::forward_data(const size_t& index) const {
    return forward_data_[index];
}

isope::fft::basic_fft::complex_t& isope::fft::fftw::backward_data(const size_t& index) {
    return backward_data_[index];
}

const isope::fft::basic_fft::complex_t& isope::fft::fftw::backward_data(const size_t& index) const {
    return backward_data_[index];
}
